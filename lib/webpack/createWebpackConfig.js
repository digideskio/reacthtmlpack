...
function requestToCommonJSModule(request /*: string*/) /*: Object*/ {
  ...
  try {
    ...
    var absPath = require.resolve(request);
...
function webpackExternalsResolver(context, requestWithLoaderStr, done) {
  var request = requestWithLoaderStr;
  if (/!/.test(requestWithLoaderStr)) {
    // We don't care about loader prefix……
    request = _lodash2.default.last(requestWithLoaderStr.split("!"));
  }
  ...
  if ((0, _path.isAbsolute)(request)) {
    ...
    var _requestToCommonJSMod = requestToCommonJSModule(request);

    var isExternal = _requestToCommonJSMod.isExternal;
    var moduleName = _requestToCommonJSMod.moduleName;
